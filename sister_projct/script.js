...
let ans_content = []
let ans_classlist = []
let ans_number = ans_content.length

function ans_updater(piece2){
    ans_number = ans_content.length
    console.log(ans_number);

    
    ans_content.push(piece2.innerHTML)
    ans_classlist.push(piece2.classList[0])

    console.log(ans_content);
    console.log(ans_classlist);

    localStorage.setItem('ans_content',JSON.stringify(ans_content))
    localStorage.setItem('ans_classlist',JSON.stringify(ans_classlist))


}

function ans_reseter() {
    ans_content = []
    ans_classlist = [] 
}


function mode_recaller() {
...
function ans_recaller() {
    let retrieved_ans = JSON.parse(localStorage.getItem('ans_content'))
    let retrieved_classlist = JSON.parse(localStorage.getItem('ans_classlist'))

    if (retrieved_ans) {
        ans.classList.remove('hidden')
        for (let letter = 0; letter < retrieved_ans.length; letter++) {
            let span  = document.createElement('span')

            span.innerHTML = retrieved_ans[letter]


            if (retrieved_classlist[letter]==='bordermaker') {

                span.classList.add('bordermaker')
...
function section_recaller() {
    let retrieved_section_order = JSON.parse(localStorage.getItem('section_listening'))
    if (retrieved_section_order) {
        console.log(retrieved_section_order);  

        if (!(retrieved_section_order[2] ==='dont_show')) {
...
function lists_2() {
    ...
    lii.forEach(l => {
        l_box.push(l.innerHTML) 
        class_box.push(l.classList)
    });

    console.log(l_box);
    
    localStorage.setItem('list__2',JSON.stringify(l_box))
    localStorage.setItem('list__2_classlist',JSON.stringify(class_box))
...
function lists_1() {
    ...
    localStorage.setItem('list__1_classlist',JSON.stringify(class_box))
                    ...
                    if (list__2.querySelector('.item3 .top-button')){
                       ...
                       if (marker_1_3rd === 1) {
                        ...
                        btn3_element.addEventListener('click', (e)=>{
                            ...
                            if((_list__2.children.length)===1 && list__2.firstElementChild.classList.contains('item3')) {
                                btn3_element.remove()
                                lists_2()
                                console.log('hiiiiiiiiiiiiiiiiiiiiiiii');
                                row2.classList.remove('shorter')
